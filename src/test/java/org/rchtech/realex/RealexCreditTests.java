...
public class RealexCreditTests {
	...
	public void creditAuthorization() throws ApiException {
		Transaction authorization = card.authorize(new BigDecimal("14")).withCurrency("USD").withAllowDuplicates(true)
				.execute();
		assertNotNull(authorization);
		assertEquals("00", authorization.getResponseCode());

		Transaction capture = authorization.capture(new BigDecimal("14")).execute();
	...
	public void creditSale() throws ApiException {
		Transaction response = card.charge(new BigDecimal("15")).withCurrency("USD").withAllowDuplicates(true)
				.execute();
	...
	public void creditSaleWithRecurring() throws ApiException {
		Transaction response = card.charge(new BigDecimal("15")).withCurrency("USD")
				.withRecurringInfo(RecurringType.Fixed, RecurringSequence.First).withAllowDuplicates(true).execute();
		assertNotNull(response);
		assertEquals("00", response.getResponseCode());
	}
	...
	public void creditRefund() throws ApiException {
		Transaction response = card.refund(new BigDecimal("16")).withCurrency("USD").withAllowDuplicates(true)
				.execute();
		assertNotNull(response);
		assertEquals("00", response.getResponseCode());
	}
	...
	public void creditRebate() throws ApiException {
		Transaction response = card.charge(new BigDecimal("17")).withCurrency("USD").withAllowDuplicates(true)
				.execute();
		assertNotNull(response);
		assertEquals("00", response.getResponseCode());

		Transaction rebate = response.refund(new BigDecimal("17")).withCurrency("USD").execute();
	...
	public void creditVerify() throws ApiException {
		Transaction response = card.verify().withAllowDuplicates(true).execute();
		assertNotNull(response);
		assertEquals("00", response.getResponseCode());
	}
	...
	public void creditFraudResponse() throws ApiException {
		...
		Transaction fraudResponse = card.charge(new BigDecimal("199.99")).withCurrency("EUR")
				.withAddress(billingAddress, AddressType.Billing).withAddress(shippingAddress, AddressType.Shipping)
				.withProductId("SID9838383").withClientTransactionId("Car Part HV").withCustomerId("E8953893489")
				.withCustomerIpAddress("123.123.123.123").execute();
	...
	public void creditSale_GB_NoStreetAddress() throws ApiException {
		...
		Transaction response = card.charge(new BigDecimal("19.99")).withCurrency("EUR")
				.withAddress(billingAddress, AddressType.Billing).execute();
		assertNotNull(response);
		assertEquals("00", response.getResponseCode());
	}
	...
	public void creditSettleWithoutAmountCurrency() throws ApiException {
		Transaction response = card.authorize(new BigDecimal("99.99")).withCurrency("EUR").execute();
		assertNotNull(response);
		assertEquals("00", response.getResponseCode());
		...
		Transaction responseSettle = settle.capture().execute();
	...
	public void creditAuthorization_WithMultiAutoSettle() throws ApiException {
		Transaction response = card.authorize(new BigDecimal("14")).withCurrency("USD").withMultiCapture(true)
				.execute();
		assertNotNull(response);
		assertEquals("00", response.getResponseCode());
	}
	...
	public void creditDccRateLookup_ChargeNotEnabledAccount() throws ApiException {
		...
		card.charge(new BigDecimal("10.01")).withCurrency("EUR").withOrderId(dccRateData.getOredrId())
				.withDccRateData(dccRateData).execute();
	...
	public void creditDccRateLookup_AuthNotEnabledAccount() throws ApiException {
		DccRateData dccRateData = card.getDccRate(DccRateType.Sale, new BigDecimal("10.01"), "EUR", DccProcessor.Fexco);
		card.authorize(new BigDecimal("10.01")).withCurrency("EUR").withOrderId(dccRateData.getOredrId())
				.withDccRateData(dccRateData).execute();
	...
	public void creditChargeGooglePay_InvalidToken() throws ApiException {
		...
		card.charge(new BigDecimal("15")).withCurrency("EUR").execute();
	...
	public void creditAuthorizationWithSupplementaryData() throws ApiException {
		Transaction response = card.authorize(new BigDecimal("129.99")).withCurrency("EUR")
	...
	public void creditSaleWithSupplementaryData() throws ApiException {
		Transaction response = card.charge(new BigDecimal("129.99")).withCurrency("EUR")
				.withSupplementaryData("taxInfo", "VATREF", "763637283332")
				.withSupplementaryData("indentityInfo", "Passport", "PPS736353").execute();
	...
	public void creditfraudManagementDecisionManager() throws ApiException {
		...
		Transaction response = card.charge(new BigDecimal("199.99")).withCurrency("EUR")
				.withAddress(billingAddress, AddressType.Billing).withAddress(shippingAddress, AddressType.Shipping)
				.withDecisionManager(decisionManager).withCustomerData(customer)
				...
				.withCustomData("fieldValue01", "fieldValue02", "fieldValue03", "fieldValue04").execute();
		assertNotNull(response);
		assertEquals("00", response.getResponseCode());
	}
	...
	public void creditFraudManagementHold() throws ApiException {
		Transaction response = card.authorize(new BigDecimal("199.99")).withCurrency("EUR").execute();
		assertNotNull(response);
		assertEquals("00", response.getResponseCode());

		Transaction transaction = Transaction.fromId(response.getTransactionId(), response.getOrderId());

		Transaction holdResponse = transaction.hold().withReasonCode(ReasonCode.Fraud).execute();
	...
	public void creditFraudManagementRelease() throws ApiException {
		...
		Transaction releaseResponse = transaction.release().withReasonCode(ReasonCode.FalsePositive).execute();
	...
	public void creditFraudFilterDataSubmission() throws ApiException {
		...
		Transaction fraudResponse = card.charge(new BigDecimal("199.99")).withCurrency("EUR")
				.withAddress(billingAddress, AddressType.Billing).withAddress(shippingAddress, AddressType.Shipping)
				.withProductId("SID9838383").withClientTransactionId("Car Part HV").withCustomerId("E8953893489")
				.withCustomerIpAddress("123.123.123.123").withFraudFilter(FraudFilterMode.Passive).execute();
		assertNotNull(fraudResponse);
		assertEquals("00", fraudResponse.getResponseCode());
	}
}
